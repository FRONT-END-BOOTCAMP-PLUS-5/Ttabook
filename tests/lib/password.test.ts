...
describe('Password 유틸리티', () => {
  ...
  describe('hashPassword', () => {
    ...
    it('빈 문자열 패스워드에 대해 에러를 발생시켜야 한다', async () => {
      ...
      await expect(hashPassword('')).rejects.toThrow('패스워드가 제공되지 않았습니다');
    });
  });
  ...
  describe('verifyPassword', () => {
    ...
    it('빈 문자열 패스워드에 대해 에러를 발생시켜야 한다', async () => {
      ...
      await expect(verifyPassword('', hashedPassword)).rejects.toThrow('패스워드가 제공되지 않았습니다');
    });
    ...
    it('빈 해시에 대해 에러를 발생시켜야 한다', async () => {
      ...
      await expect(verifyPassword(plainPassword, '')).rejects.toThrow('해시된 패스워드가 제공되지 않았습니다');
      await expect(verifyPassword(plainPassword, '   ')).rejects.toThrow('해시된 패스워드가 제공되지 않았습니다');
    });
  });
